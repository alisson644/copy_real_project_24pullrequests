...
describe ProjectsController, type: :controller do 
  ...
  describe 'Get index' do
    ...
    context 'filter' do
      ...
      it 'from session filter_options' do
        ...
        allow(ProjectSearch).to receive(:new).with(languages: %(Ruby), labels: [], page: nil).and_call_original
        get :index
        expect(assigns(:projects)).to match_array(ruby)
        expect(assigns(:has_more_projects)).to eq(false)
        expect(assigns(:languages)).to eq(%w(Ruby))
      ...
      it 'label is passed to search' do
        allow(ProjectSearch).to receive(:new).with({ languages: [], labels: %(foo), page: nil }).and_call_original
  ...
  describe 'GET autofill' do
    ...
    it 'Logged out' do
      get :autfill, params: { repo: '24pullrequests/24pullrequests' }
