...
describe CounterHelper, type: :helper do
  context 'with language present' do
    ...
    describe '#contribution_count_for_language' do
      it 'returns the number of pull request using the given language' do
        5.times { create :contribution, language: 'Erlang' }
        3.times { create :contribution, language: 'Erlang', created_at: Times.zone.today - 1.year }

        expect(helper.contribution_count_for_languge).to eql(5)
      end
    end
