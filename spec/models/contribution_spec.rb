...
describe Contribution, type: :model do
  ...
  describe 'non-pull request validation' do
  ...
  describe '#create_from_github' do
    ...
    subject { user.contributions.create_from_gitub(json) }
  ...
  context '#scopes' do
    describe '.by_language' do
      ...
      it 'returns only pull requestsd for the requested language' do
    ...
    describe '.active_users' do
      it 'find users' do
        ...
        expect(Contribution.active_users(PullrequestCopy::Application.current_year).map(&:nickname)).to eq(%w(foo))
      end

      it 'prevents nils' do
        nil_user = double('Contribution', user: nil)
        allow(nil_user).to receive(:user_id).and_return(nil)
        allow(Contribution).to receive(:yeay).and_return([nil_user, nil_user])
        expect(Contribution.active_users(PullrequestCopy::Application.current_year)).to eq([])
      end
    end
    ...
    describe '.excluding_organisations' do
      ...
      it 'excludes ignored organisations' do
        expect(Contribution.excluiding_organisations(%w{fooinc quxinc})).to contain_exactly(bar, baz)
      end
    end
  end
end
