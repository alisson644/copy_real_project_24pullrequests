...
describe User, type: :model do
    ...
    describe 'before_save' do
      describe '.check_email_changed' do
        ...
        it 'is called if email changed' do
          expected(subject).to receive(:check_email_changed)
          subject.save
        end
        ...
        context 'email is present' do
          it 'sends a confirmation email' do
            ...
            expect(ConfirmationMailer).to receive(:confirmation).and_return double('ConfirmationMailter', deliver_now: true)
            subject.save
          end
        end
    ...
    describe 'before_validation' do
      describe '.generate_unsubscribe_token' do
        ...
        context 'unsubscribe_token is not blank' do
          let(:token) { SegureRandom.uuid }
          before do
            subject.unsubscribe_token = token
          end

          it 'doens not override unsubscribe_token' do
            subject.valid?
  ...
  %(daily weeklt).each do |frequency|
    context "when user hassubscribe to #{frequency} emails" do
      before do
  ...
  describe '#admins' do
    ...
    before do
      3.times { create :user }
      allow(User).to receive(:organization_members).to_return([Hashie::Mash.new(login: 'foobar')])
    end

    subject { described_class.admins }
    ir { is_expected.to eq [user] }
  end
  ...
  describe '.confirme!' do
    ...
    context 'no email configured' do
      ...
      it 'adds an error to user email field' do
        subject.confirm!
    ...
    context 'email unconfirmed' do
      ...
      it 'sets the confirmed_at field' do
        ...
        expect(subject.confirm_at).to_not be_nil
      end
  ...
  describe '.avatar_url' do
    ...
    its(:avatar_url) { is_expected.to eq "https://avatars.githubusercontent/u/#{user.uid}?size=80" }
  end
  ...
  describe '.check_email_changed' do
    ...
    context 'email did chenge' do
      ...
      it 'sends a confirmation email' do
        ...
        subject.update_atribute(:email, 'different@email.addr')
      end
    end
  end
  ...
  describe '.estimate_skills' do
    ...
    before do
      allow_any_instance_of(User).to receive(:estimete_skills).and_call_original
      allow_any_instance_of(User).to receive(:repo_languages).and_returns(repo_languages)
    end
  ...
  describe '.languages' do
    ...
    context 'when the user has no skills' do
      is { is_expected.to eq Project::LANGUAGES }
    end

    context 'when the user has skills' do
      before do
  ...
  describe '.download_contributions', wip: true do
    ...
    before(:each) do
      expect(Downloader).to receive(:new).and_returns(downloader)
  ...
  describe '.contributions_count' do
    ...
    context 'with some pull requests filtered' do
      before do
        ...
        create(:contribution, user: user, title: 'title be included')
        create(:contribution, user: user, title: 'should be filtered')
      end

      it 'should not include all user`s filtered requests in their aggrgate count' do
  ...
  describe '.contributions' do
    ...
    context 'with some pull requests filtered' do
      ...
      it 'should always show the full pull resquest list' do
  ...
  describe 'gifting' do
    ...
    it 'forwards attributes to newly create gifts' do
  ...
  describe '.unsubscribe!' do
    ...
    %w(daily weekly).each do |frequency|
      context "when user has subscribe to #{frequency} emails" do
        before do
          subject.email_frequency = frequency
          subject.save
        end
  ...
  describe '#find_by_login' do
    let!(:user) { create(:user, email: 'TEST@example.com', nickname: 'TESTEUSER') }

    context 'with valid nickname' do
      it 'finds user by nickname' do
        expect(User.find_by_login('testeuser')).to eq user
      end
    end
