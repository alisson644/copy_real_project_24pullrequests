...
describe GiftForm, type: :model do
  ...
  describe '.contributions_for_select' do
    ...
    context 'when a pull request has been gifted' do
      ...
      it { is_expected.to include ["Gifted: #{contribution.repo_name} - #{contribution.title}", contribution.param] }
    end

    context 'when a pull request is old' do
      it { is_expected.not_to include ["Not gifted: #{old_contribution.repo_name} - #{old_contribution.title}", old_contrubution.to_param] }
    end
  end

  describe '.show_date_selected?' do
    subject { gift_form.show_date_selected? }
