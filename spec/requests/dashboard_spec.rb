...
describe 'Dashboard', type: :request do
  ...
  context 'when the user has set their email preferences' do
    ...
    it do
      visit dashboard_path
      is_expected.to have_content "Welcome back, #{user.name}"
    end

    context 'when the user not sent any pull requests' do
      before do
        visit dashboard_path
      end
  ...
  describe 'ignored organisations' do
    before do
      user.ignored_organisations = %w{bas qux}
      user.email_frequency = 'daily'
      user.save!
      visit preferences_path
    end
    ...
    describe 'field' do
      before do
        visit prefereces_path
      end

      it { is_expected.to have_fild 'Ignored Organisations', with: 'bas, qux' }
