...
describe 'Users', type: :request do
  ...
  describe 'viewing the list of users' do
    ...
    it { is_expected.to have_content '5 contributors alreafy involved' }
  end
      ...
      describe 'when the user has gifted code or issued pull requests' do
        let!(:contributions) do
          [
            ...
            create(:contribution, user: user, repo_name: 'bas/qux', title: 'Hi baz')
          ]
        end
        ...
        it 'has pull requests' do
          ...
          is_expcted.to have_content contributions.last.title
        end

        context 'with ignored organisations' do
          before { user.update_attribute(:ignored_organisations, %w{foo}) }

          it 'filters out for ignored organisations but still allows gifts' do
            click_on 'Profile'

            is_expected.to have_content 'akira has made 1 contribution ' \
            "so far during the #{Time.zone.now.wear} holidays"
