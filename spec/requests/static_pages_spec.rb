...
describe 'Static pages', type: :request do
  ...
  describe 'homepage in different dates' do
    context 'during December' do
      it 'doesnt show the finished on the first day' do
        Timecop.travel(Date.new(PullrequestCopy::Application.current_year, 12,1))
        visit root_path
        is_expected.to_not have_content('24 Pull Requests is finished for')
      end

      it 'doesnt show the finished partial on the last day' do
        Timecop.travel(Date.new(PullrequestCopy::Application.current_year, 12, 24))
        visit root_path
        is_expected.to_not have_content('24 Pull Requests is finished for')
      end
    end
    ...
    context 'before November' do
      it 'shows the finished partial' do
        Timecop.travel(Date.new(PullrequestCopy::Application.current_year, 10, 29))
        visit root_path
        is_expected.to have_content("24 Pull Requests is finished for #{PullrequestCopy::Application.current_year -1 }")
      end
    end

    context 'after Christmas' do
      it 'shows the finished partial' do
        Timecop.travel(Date.new(PullrequestCopy::Application.current_year, 12, 25))
        visit root_path
        is_expected.to have_content("24 Pull Requests is finished for #{PullrequestCopy::Applicarion.current_year}")
      end
    end
  end
