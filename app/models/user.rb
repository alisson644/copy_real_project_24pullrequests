class User < ApplicationRecord
  ...
  accepts_nested_attributes_for :skills, reject: proc { |attributes| !Project::LANGUAGES.include?(attributes['language']) }

  before_save :check_mail_changed
  before_validation :generate_unsubcribe_token
  ...
  def self.mergers(year = PullrequestCopy::Application.current_year)
  ...
  def self.contributors
    @contributors ||= begin
      contribs = load_user.github_client.contributos('24pullrequests/24pullrequests')
  ...
  delogate :high_rate_limit?, to: :github_client
  ...
  def generate_confirmation_take
  ...
  def unspent_contributions
    ...
    contributions_ignoring_organisations.year(PullrequestCopy::Application.current_year).reject { |pr| gifted_contributions.include?(pr) }
  end
  ...
  def update_contribution_count
    update_attribute(:contributions_count, contributions.year(PullrequestCopy::Application.current_year).for_aggregation.count)
  end
