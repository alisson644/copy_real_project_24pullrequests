class Contribution < ApplicationRecord
  belongs_to :user
  belongs_to :merger, class_name: 'User', foreing_key: :merged_by_id, primary_key: :uid
  after_save { if user then user.update_contribution_count end }
  after_destroy { if user then user.update_contribution_count end }

  validates :issue_url, uniqueness: { scope: :user_id}, if: :pull_requests?

  validates :body, presence: true, unless: :pull_requests?
  validates :repo_name, presence: true, unless: :pull_requests?
  validates :create_at, presence: true, unless: :pull_requests?
  ...
  EARLIEST_PULL_DATE = Date.parse("01/12/#{PullrequestCopy::Application.current_year}").midnight
  LATEST_PULL_DATE   = Date.parse("25/12/#{PullrequestCopy::Application.current_year}").midnight
  ...
  def post_to_firehose
    ...
    return unless created_at.year == PullrequestCopy::Application.current_year
  ...
  def post_twitter
    ...
    Rails.logger.error "likely a Twittwe API error occured:\n"\
                        "#{e.inspect}"
  end

  def gifted_state
    gift.any? ? :gifted : :not_gifted
  end
