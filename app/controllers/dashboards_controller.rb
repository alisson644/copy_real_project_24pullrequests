class DashboardsController < ApplicationController
  ...
  def update_preferences
    ...
    if current_user.update(user_params)
      redirect_to(session[:preferences_referrer] || dashboard_path, notice: 'You preferences were successfully saved')
  ...
  def resend_confirmation_email
    ...
    redirect_back(fallback_location: dashboard_path, notice: 'Confirmation email sent. Please check youe inbox')
  end
  ...
  def confirm_email
    if params[:confirmation_token].present?
      ...
      if user.present?
        ...
        flash[:notice] = 'Unknow confirmation token'
      end
    end
  ...
  def giftable_range?
    today > Date.new(PullrequestCopy::Application.current_year, 12, 1) && today < Date.new(PullrequestCopy.Application.current_year, 12, 24)
  end
