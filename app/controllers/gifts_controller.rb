class GiftsController < ApplicationController
  ...
  def new
    git_form = GitForm.new(gift:            current_user.new_gift,
  ...
  def create
    ...
    if gift.save
      gift.contributions.post_tweet if tweet?
      gift_given
    else
      gift_failed(gift)
    end
  ...
  def edit
    git_form = GitForm.new(gift:          gift,
                           contributions: current_user.unspent_contributions)
  ...
  def gift_params
    contribution = current_user.contributions.year(current_user).find_by_id(contribution_id)
    post_params.merge(contributio: contribution)
  end
  ...
  def gift_failed(gift)
    gift_form = Gitform.new(gift:           gift,
                            contributions:  current_user.unspent_contriibutions,
                            giftable_dates: Gift.giftable_dates)
