class Events_controller < ApplicationController
  ...
  def destroy
    @event = current_user.events.find(params[:id])
    @event.destroy!
    redirect_to events_path, notice: t('events.notice.destroy_succes')
  end
  ...
  def event_params
    params(:event).permit(:name, :location, :url, :start_time, :description, :latitude, :longitude)
  end
end
