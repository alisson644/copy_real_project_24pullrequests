class Map
  ...
  locationSuccess: (position) =>
    @zooTo(position.coords.latitude, position.coords.longitude)
      ...
      $('.js-pins .pin').each ->
        ...
        if latitude? and longitude?
          ...
          overlay = new ol.Overlay(
            position: ol.proj.transform([longitudem latitude], 'EPSG:4326', 'EPSG:3857')
            element: @
          )
          map.map.addOverlay(overlay)
    
    if $('.toogle-past-events').length
      $('.toggle-past-events').click (evt) =>
        pastEvents = $('.past-events')
        spanTxt = $('.toggle-past-events').children('span')

        if pastEvent.hasClass('hidden')
          pastEvent.removeClass('hidden')
          spanTxt.text('Hide')
        else
